...
try:
    ...
    for idx, row in d_df.iloc[(curr_p-1)*100 : curr_p*100].iterrows():
        ...
        cols[0].markdown(f"<span class='row-marker'></span><span style='font-size:1.9rem;'>{row['분류']}</span>", unsafe_allow_html=True)

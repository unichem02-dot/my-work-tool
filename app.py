...
if st.query_params.get("study") == "true":
    ...
    try:
        ...
        # 프론트엔드로 전달할 전체 데이터 딕셔너리 변환 (sheet_idx 추가)
        study_data = df[['분류', '단어-문장', '해석', '발음', '메모1', '메모2', 'sheet_idx']].rename(
...
if not st.session_state.authenticated and st.session_state.logging_in:
    ...
    # ==========================================
    # 🌟 메인 앱 상단 UI (세련되고 편리한 대시보드 구조 복구)
    # ==========================================
    ...
    if st.session_state.app_mode == 'English':
        try:
            ...
            if st.session_state.authenticated:
                ...
                cb[2].download_button("📥 CSV 추출", df.to.csv(index=False).encode('utf-8-sig'), f"English_Data_{time.strftime('%Y%m%d')}.csv", use_container_width=True)
